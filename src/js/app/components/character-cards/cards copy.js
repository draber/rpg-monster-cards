...
import monsters from '../../../../data/characters.json';

const cardStore = JSON.parse(localStorage.getItem(config.storageKeys.card) || '{}');

let area;
let smoke;

let autoincrement = (Math.max(...Object.keys(cardStore)) || 0) + 1

const edit = e => {
    const card = e.target.closest('card-container');
    card.classList.add('edit');
    fn.$$('value, badge', card).forEach(field => {
